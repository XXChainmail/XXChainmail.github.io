!function() {
  ...
  function Calendar(selector, events) {
    ...
    this.events = events.map(function(ev) {
      return {
        ...
        date: ev.date ? (moment.isMoment(ev.date) ? ev.date.clone() : moment(ev.date)) : null,
        location: ev.location || 'TBA',
        time: ev.time || 'TBA',
        specialDetails: ev.specialDetails || ''
      };
    });
  ...
  Calendar.prototype.renderEvents = function(events, ele) {
    ...
    events.forEach(function(ev) {
      ...
      div.addEventListener('click', function(e) {
        e.stopPropagation();
        // If a pageId is provided, navigate to that page on click.
        if(ev.pageId) {
          window.location.href = ev.pageId;
          return;
        }
        window.selectedEvent = ev;
        window.location.href = '/apps/event.html?event=' + encodeURIComponent(ev.eventName);
      });
      
      var square = createElement('div', 'event-category green');
  ...
  // Legend removed: events are now rendered in a single default green style.
